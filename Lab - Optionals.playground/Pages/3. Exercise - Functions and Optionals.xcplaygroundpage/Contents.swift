...
print(returnAge(age: userInputAge))
...
func costOfItem(item: String) -> Double? {
    if let itemInStock = stock[item] {
        if itemInStock > 0 {
            print("Price of \(item) is \(prices[item]!).")
            return prices[item]
        }
    }
...
costOfItem(item: "Donuts")
costOfItem(item: "Juice")
costOfItem(item: "Cottage Cheese")
/*:
