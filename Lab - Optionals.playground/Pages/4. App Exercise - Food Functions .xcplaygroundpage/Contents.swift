...
func mealLog(mealType: String) -> Meal? {
    if let unwrappedMeal = meals[mealType] {
        print("Unwrapped is \(unwrappedMeal).")
        return meals[mealType]
    } else {
        print("You don't have \(mealType) in meals.")
        return nil
    }
}
